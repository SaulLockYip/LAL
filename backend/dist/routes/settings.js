...
settingsRouter.get('/voices', async (_req, res) => {
    try {
...
settingsRouter.put('/user', async (req, res) => {
    try {
        const { name, nativeLanguage, targetLanguage, currentLevel, voice, ttsApiKey, ttsVoiceId, ttsSpeed, ttsVol, ttsPitch, ttsEmotion, ttsAudioSampleRate, ttsBitrate, ttsChannel, ttsSoundEffects } = req.body;
        ...
        if (isTTSOnlyUpdate && !existingUser) {
            res.status(400).json(errorResponse('VALIDATION_ERROR', 'User must be created first before saving TTS settings'));
            return;
        }
        let user;
        if (existingUser) {
...
settingsRouter.get('/tts-voices', async (req, res) => {
    try {
